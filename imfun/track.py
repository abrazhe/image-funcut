...
def follow_extrema(arr, start, mode='gany', memlen=5):
    ...
    """
    def _ext(v):
        ##(xf,yf),(mx,mn), (gups, gdowns) = core.extrema.extrema2(v)
...
class LCV_Contours:
    ...
    def update_accelerations(self):
        ...
        for j in xind[:]:
            ...
            c1 = np.sum(self.U[inside,j])/np.sum(inside+1e-12)
            c2 = np.sum(self.U[outside,j])/np.sum(outside+1e-12)
